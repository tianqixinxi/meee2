...
**File** `Sources/Board/SessionArtifactsAPI.swift`

The session artifact endpoint is a read-only projection of canonical `PlannerArtifact` records produced by planner nodes bound to that session. Hook and terminal events never create artifact records.

| Route | Response |
|---|---|
| `GET /api/sessions/:sessionId/artifacts` | `{sessionId, artifacts, totalCount}` |
