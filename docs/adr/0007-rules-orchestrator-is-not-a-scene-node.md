...
For Poker Table, GM and Dealer remain scene roles, not the control plane. GM is a human responsibility node for rulings, approvals, and exceptional cases. Dealer is the node-scoped home for authoritative `game-state.json` and `action-log.json`, but the system may submit those artifacts on behalf of the Rules Orchestrator.
