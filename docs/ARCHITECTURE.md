...
Hook events update session runtime state only. Artifacts are created explicitly by planner node output and are never inferred from terminal commands, tool calls, or final assistant messages.
