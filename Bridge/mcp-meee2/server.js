...
// 把产物结构化回传。
const INSTRUCTIONS = [
  'You are running inside a meee2 native Claude/Codex session integration.',
  'Meee2 Runtime is the source of truth for planner nodes and artifacts.',
  'Use the tools below to understand your assigned node and submit structured',
  'results back to the canvas.',
  '',
  'READ / OBSERVE:',
  '  - read_inbox     — pending direct messages addressed to you',
  '  - list_sessions  — all live Claude sessions',
  '  - read_node_contract — planner node inputs/outputs/artifact schema',
  '',
  'SEND:',
  '  - submit_node_output — final structured planner node output',
  '  - attach_artifact_to_node — attach intermediate planner evidence',
  '',
  ...
  '  - If your task says you are executing a meee2 planner node, call',
  '    read_node_contract first. When complete or blocked, call',
  '    submit_node_output. Do not rely on terminal text alone for planner',
  '    artifacts. For large text/html/json/file outputs, write the file inside',
  '    your cwd and submit payload.file.path rather than pasting the full body.',
  '',
