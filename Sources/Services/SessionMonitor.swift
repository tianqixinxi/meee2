...
class SessionMonitor: ObservableObject {
    ...
    /// 解析队列
    private let parseQueue = DispatchQueue(label: "com.peerisland.sessionparse", qos: .userInitiated)
    ...
    private func loadSessionsFromDirectory() -> [AISession] {
        ...
        NSLog("[SessionMonitor] Found \(files.count) files in sessions directory")
        ...
        for fileURL in files {
            ...
            if let session = parseSessionFile(fileURL) {
                NSLog("[SessionMonitor] Loaded session: \(session.id), pid: \(session.pid)")
