...
public struct SessionData: Codable, Identifiable {
    ...
    public var customTitle: String?         // 用户显式重命名，优先于 provider / 自动标题
...
public class SessionStore: ObservableObject {
    ...
    public func upsert(_ session: SessionData) {
        ...
        if let ex = existing {
            ...
            if merged.customTitle == nil {
                merged.customTitle = ex.customTitle
            }
    ...
    private static func mergeContinuity(from old: SessionData, into target: SessionData) -> SessionData {
        ...
        if merged.customTitle == nil { merged.customTitle = old.customTitle }
    ...
    private func rebaseSessionChanges(
        ...
        if candidate.customTitle != baseline.customTitle { rebased.customTitle = candidate.customTitle }
    ...
    private func hasSessionChanged(
        ...
        if old.customTitle != new.customTitle { return true }
...
enum SessionDataMigrations {
    ...
    private static func step(_ s: SessionData, from v: Int) -> SessionData {
        ...
        case 3:
            ...
            return s
        case 4:
            // v4 → v5：新增 custom_title。旧记录保持 nil；用户显式重命名后
            // 由本地 Board API 写入，并始终优先于 provider / 自动标题。
            return s
        default:
            return s
        }
