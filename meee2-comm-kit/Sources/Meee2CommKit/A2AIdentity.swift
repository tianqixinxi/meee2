...
///   3. 进程 cwd 精确匹配宿主注入的 `IdentityResolver` 中的 `project`
///
...
public struct A2AIdentity {
    ...
    /// 主仓在启动时注入。未注入时,cwd 路径上的解析返回 nil。
    public nonisolated(unsafe) static var resolver: IdentityResolver?
    ...
    public static func currentSessionId() -> String? {
        ...
        let matches = resolver?.sessionsForCwd(cwd) ?? []
        if matches.count == 1 {
            return matches[0]
        }
        if matches.count > 1 {
            commLog(.debug, "[A2AIdentity] cwd '\(cwd)' matched \(matches.count) sessions - ambiguous")
        }
        return nil
    }
